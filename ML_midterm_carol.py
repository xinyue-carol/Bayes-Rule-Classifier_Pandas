...
train_df=datadf[0:int(0.75*float(len(datadf)))]
test_df=datadf[int(0.75*float(len(datadf)+1)):int(len(datadf))]
#print(len(train_df))
#print(len(test_df))
...
for i in range(len(t_a_list)):
    if t_a_list[i][0]!=t_a_list[i][1]:
        if t_a_list[i][0]==1:
            pdc_1c.loc[pdc_1c['d']==tuple(td_list[i]),'%']+=delta
        if t_a_list[i][0]==2:
            pdc_2c.loc[pdc_2c['d']==tuple(td_list[i]),'%']+=delta
        if t_a_list[i][0]==3:
            pdc_3c.loc[pdc_3c['d']==tuple(td_list[i]),'%']+=delta
    else:
        continue
...
for i in range(len(td_list)):
    a=(td_list[i])
    p1=e_1*float(pdc_1c.loc[pdc_1['d']==tuple(a),'n_p'].values)*p_c1
    p2=e_2*float(pdc_2c.loc[pdc_2['d']==tuple(a),'n_p'].values)*p_c2
    p3=e_3*float(pdc_3c.loc[pdc_3['d']==tuple(a),'n_p'].values)*p_c3
    if p1> p2 and p1>p3:
